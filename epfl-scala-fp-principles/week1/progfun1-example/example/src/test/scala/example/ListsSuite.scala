...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of a negative numbers (10pts)") {
    assertEquals(sum(List(1,2,-7)), -4)
  }

  test("sum of an empty list (10pts)") {
    assertEquals(sum(List.empty), 0)
  }
  ...
  test("max of negative numbers (10pts)") {
    assertEquals(max(List(-3, -7, -2, -3, 0)), 0)
  }

  test("max throws an exception if list is empty (10pts)") {
    try
      max(List.empty)
      fail("No exception has been thrown")
    catch
